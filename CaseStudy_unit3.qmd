...
-   There are multiple date fields, date.x and date.y, both of which combine year, month, and day together in one field.
...
-   All data is joined, even if it doesn't have anything in a field (Vin has no n_stem_classes data)
...
```{r}
joined_data <- full_join(gradebook, time_spent, by = join_by(student_id, subject, semester, section))
...
-   All columns from both data sets were included in the join, leaving some fields with no data as one data set had no data for those fields. Rows with missing data are arranged below the fields with full data.
...
-   There is a difference in the type of data for that column, even though the column name is the same. Data in one is a character, data in the other is a double. Doubles are for numeric data, but characters can be any letter or number.
...
-   There's quite a few missing variables in some categories, including gender, course_id, and the date fields which could be important when doing analysis.
...
-   Raising the number of bins increased detail of the histogram, but past a certain point the extra detail wasn't really revealing much more. So I think 50 bins was about the right amount of detail versus simplification.
...
```{r}
ggplot(data_to_explore) +
  geom_histogram(aes(x = time_spent_hours), bins = 50, colour = "red") + 
  facet_wrap(~subject)
```

What do you notice about this figure? What questions do you have? Add a note (or a few notes!) below:

-   Why does FrScA have so much more counts than the others? Are the total counts for each subject equal enough for this kind of comparison? Also PhysA and BioA seems really low for those types of classes.
...
-   There's a cluster at about 0,0; are these NA entries or actual scores? There is nothing controlling the range for the x and y, are these generated automatically by ggplot?
...
-   This series of graphs shows a relationship between the number of hours spent on a course learning management system compared against the proportion of total points earned in the course. Each graph represents a separate course, and the color of the points shows the student's enrollment status. The graphs all show a marked increase in the likelihood of a higher grade with more hours spent in the LMS, with the greatest clustering near 1.00 at about 25 hours.
...
```{r}
data_to_explore |> 
  select(time_spent_hours, post_int, post_percomp, proportion_earned) |> 
  correlate() |> 
  rearrange() |>
  shave() |>
  fashion()
...
```{r}
m2 <- lm(proportion_earned ~ time_spent_hours + tv, data = data_to_explore)
...
-   Changing a variable to tv, which I believe is self-reported time spent watching tv per day, is also a statistically significant factor to proportion of total grade earned. It seems this would suggest that more tv was related to higher grades when I would have expected an inverse relationship, but I'm not sure with this method if the nature of relationship is indicated or not. I could also be wrong about what tv means.
